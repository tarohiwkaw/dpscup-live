(function(){
  ...
  function render(v){
    ...
    document.querySelectorAll('[data-final-weather]').forEach(el=>{
      ...
      el.innerHTML='<div class="weather-top"><span>🌧️</span><span class="weather-status">DPSCUP WEATHER UPDATE</span><span>💧</span></div><div class="weather-main">'+title+'</div><div class="weather-sub">'+sub+'</div>'+(postponed?'<div class="weather-date">📅 ย้ายไปแข่งพรุ่งนี้ · '+esc(date)+(v.time?' · '+esc(v.time):'')+'</div>':'<div class="weather-date">⏳ รอติดตามสถานการณ์คืนนี้</div>');
    });
  }
  function start(){
